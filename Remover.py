...
class CanFG(nn.Module):
    ...
    def trainG(self, img_a):
       ...
       # 2、身份偏离损失
        ...
        emb_fake = self.FR(F.interpolate(img_fake, (112, 112), mode='bilinear', align_corners=True))
        #todo 平均损失
        mean_identities=self.mean_identity.repeat(len(emb_img), 1)
        id_loss =  torch.cosine_similarity(emb_img ,emb_fake ).mean()+1-torch.cosine_similarity(mean_identities,emb_fake).mean()
